...
class TestUsers(unittest.TestCase):
    ...
    def test_user_signup(self):
        ...
        rs = self.client.post(path + 'auth/signup', data=json.dumps({
            "name": "", "username": "litwachi", "password": "12345678", "email": "katuka.wilfred8@gmail.com"}),
                              content_type='application/json')
        self.assertEqual(rs.status_code, 400)

        # test name validation
        rs = self.client.post(path + '/auth/signup', data=json.dumps({
            "name": "Wilfred Katuka", "username": " ", "password": "12345678", "email": "katuka.wilfred8@gmail.com"}),
                              content_type='application/json')
        self.assertEqual(rs.status_code, 400)

        # test password
        rs = self.client.post(path + '/auth/signup', data=json.dumps({
            "name": "Wilfred Katuka", "username": "litwachi", "password": " ", "email": "katuka.wilfred8@gmail.com"}),
                              content_type='application/json')
        self.assertEqual(rs.status_code, 400)
        # tests email validation
        rs = self.client.post(path + '/auth/signup', data=json.dumps({
            "name": "Wilfred Katuka", "username": "litwachi", "password": "12345678", "email": " "}),
                              content_type='application/json')
        self.assertEqual(rs.status_code, 400)

        # user signup
        rs = self.client.post(path + '/auth/signup', data=json.dumps({
            "name": "tester", "username": "tester", "password": "12345678", "email": "tester@gmail.com"}),
                              content_type='application/json')
    ...
    def tearDown(self):
        ...
        user_id = user_data[0]
        db.query("DELETE from products WHERE user_id=%s", [user_id])
